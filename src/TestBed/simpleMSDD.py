...
class SimpleMSDD:
    ...
    def systematic_expand(self, cur_node, wp, ws, delta, tp, ts):
        ...
        prec_list = []
        succ_list = []
        prec_list = self.create_msdd_token(tp, start_time=0)
        succ_list = self.create_msdd_token(ts, start_time=wp + delta)
        # for i, tok in enumerate(tp):
        #     prec_list.append(self.create_msdd_token(tok))
    ...
    def f_max(self, node, stream, delta, wp, ws):
        ...
        for i in range(len(stream)):
            ...
            prec = self.create_msdd_token(prec, 0)
            succ = self.create_msdd_token(succ, i + wp + delta)
